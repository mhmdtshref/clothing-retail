...
export async function GET(req) {
  ...
  for (const c of items) {
    ...
    const company =
      chosen === providers.optimus ? 'optimus' :
      (chosen === providers.sabeq_laheq ? 'sabeq_laheq' : undefined);
  ...
  for (const [prov, set] of cityIdsByProvider.entries()) {
    ...
    if (ids.length) {
      const rows = await City.find({ provider: prov, providerCityId: { $in: ids } }, { providerCityId: 1, name: 1 }).lean();
  ...
  for (const [prov, set] of areaIdsByProvider.entries()) {
    const ids = Array.from(set.values());
    if (ids.length) {
      const rows = await Area.find({ provider: prov, providerAreaId: { $in: ids } }, { providerAreaId: 1, name: 1 }).lean();
  ...
  const results = items.map((c) => {
    ...
    const company =
      chosen === providers.optimus ? 'optimus' :
      (chosen === providers.sabeq_laheq ? 'sabeq_laheq' : undefined);
    const resolvedCityName = (chosen && chosen.cityName) || (chosen && typeof chosen.providerCityId === 'number'
      ? (cityNameMap.get(`${company}:${chosen.providerCityId}`) || undefined)
      : undefined);
    const resolvedAreaName = (chosen && chosen.areaName) || (chosen && typeof chosen.providerAreaId === 'number'
    ...
    return {
      ...
      phone: c.phone,
      provider: {
        cityId: (chosen && typeof chosen.providerCityId === 'number') ? chosen.providerCityId : undefined,
        areaId: (chosen && typeof chosen.providerAreaId === 'number') ? chosen.providerAreaId : undefined,
...
export async function POST(req) {
  ...
  try {
    ...
    if (hasProviderFields) {
      ...
      const cityIdNum = typeof raw.providerCityId !== 'undefined' ? Number(raw.providerCityId) : undefined;
      const areaIdNum = typeof raw.providerAreaId !== 'undefined' ? Number(raw.providerAreaId) : undefined;
      const addrLineStr = typeof raw.addressLine !== 'undefined' ? String(raw.addressLine || '') : undefined;
      let cityNameStr = typeof raw.providerCityName !== 'undefined' ? String(raw.providerCityName || '') : undefined;
      let areaNameStr = typeof raw.providerAreaName !== 'undefined' ? String(raw.providerAreaName || '') : undefined;

      // Fallback: if names not provided, try to resolve from City/Area collections
      try {
        if (!cityNameStr && typeof cityIdNum === 'number') {
          const cityDoc = await City.findOne({ provider: company, providerCityId: cityIdNum }, { name: 1 }).lean();
          if (cityDoc?.name) cityNameStr = cityDoc.name;
        }
        if (!areaNameStr && typeof areaIdNum === 'number') {
          const areaDoc = await Area.findOne({ provider: company, providerAreaId: areaIdNum }, { name: 1 }).lean();
    ...
    if (err?.code === 11000) {
      ...
      const c = phone ? await Customer.findOne({ phone }).lean().exec().catch(() => null) : null;
      if (c) {
        return NextResponse.json(
          {
            ok: true,
