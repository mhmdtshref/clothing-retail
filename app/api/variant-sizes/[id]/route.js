...
import { VariantSizeUpdateSchema } from '@/lib/validators/variant-size';
...
export async function PATCH(req, context) {
  ...
  try {
    ...
    const parsed = VariantSizeUpdateSchema.safeParse(body);
    if (!parsed.success) {
  ...
  try {
    ...
    if (typeof update.nameKey !== 'undefined') {
      const existing = await VariantSize.findOne({ _id: { $ne: id }, nameKey: update.nameKey })
        .lean()
        .exec();
      if (existing) {
        return NextResponse.json(
