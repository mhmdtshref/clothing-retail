...
export default function ReceiptsListPage({ companies }) {
  ...
  }, [state.query, state.status, state.companyId, state.sort, state.order, state.page, state.limit, state.dateFrom, state.dateTo]);
  ...
  const openStatusMenu = (evt, row) => { setMenuAnchor(evt.currentTarget); setMenuRow(row); };
    ...
    <Paper sx={{ p: 2 }}>
      <Toolbar sx={{ gap: 1, flexWrap: 'wrap' }}>
        ...
        <FormControl size="small" sx={{ minWidth: 140 }}>
          ...
          <Select labelId="status-label" label="Status" value={state.status} onChange={onStatusChange}>
        ...
        <FormControl size="small" sx={{ minWidth: 180 }}>
          ...
          <Select labelId="company-label" label="Company" value={state.companyId} onChange={onCompanyChange}>
        ...
        <FormControl size="small" sx={{ minWidth: 160 }}>
          ...
          <Select labelId="sort-label" label="Sort by" value={state.sort} onChange={onSortChange}
        ...
        <Button component={Link} href="/receipts/new" startIcon={<AddIcon />} variant="contained">New Purchase</Button>
        ...
        {!loading && !error && (
          <>
            <Table size="small">
              ...
              <TableBody>
                {data.items.length === 0 && (
                  <TableRow><TableCell colSpan={6}><Typography color="text.secondary" sx={{ py: 3 }}>No receipts found.</Typography></TableCell></TableRow>
                )}
                  ...
                  return (
                    <TableRow key={r._id} hover>
                      ...
                      <TableCell align="right">
                        <Tooltip title={disabledEdit ? 'Completed receipts cannot be edited (Step 4 adds editor)' : 'View / Edit (coming in Step 4)'}>
                          <span>
                            <Button size="small" startIcon={<EditIcon />} disabled>{'View / Edit'}</Button>
                          </span>
                        </Tooltip>

                        <Tooltip title={disabledStatus ? 'Completed receipts cannot change status' : 'Change Status'}>
                          <span>
                            <Button size="small" onClick={(e) => (!disabledStatus ? openStatusMenu(e, r) : null)} disabled={disabledStatus}>
                              Change Status
                            </Button>
                          </span>
                        </Tooltip>
            ...
            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ px: 2, py: 2 }}>
              ...
              <Pagination page={state.page} count={data.meta.pages || 1} onChange={onPageChange} color="primary" shape="rounded" showFirstButton showLastButton />
            </Stack>
          </>
        )}
      </Box>
      ...
      {/* Status change menu */}
      <Menu anchorEl={menuAnchor} open={openMenu} onClose={closeStatusMenu} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} transformOrigin={{ vertical: 'top', horizontal: 'right' }}>
        <MenuItem onClick={() => setStatus('ordered')}><ListItemIcon><PendingActionsIcon fontSize="small" /></ListItemIcon>ordered</MenuItem>
        <MenuItem onClick={() => setStatus('on_delivery')}><ListItemIcon><LocalShippingIcon fontSize="small" /></ListItemIcon>on_delivery</MenuItem>
        <MenuItem onClick={() => setStatus('completed')}><ListItemIcon><DoneIcon fontSize="small" /></ListItemIcon>completed</MenuItem>
      </Menu>
    </Paper>
  );
}
