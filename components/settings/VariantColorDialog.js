...
export default function VariantColorDialog({
  open,
  onClose,
  onSaved,
  initialValue,
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const conflictLocal = (existingColors || []).some((c) => {
        const id = String(c?._id || '');
        if (currentId && id === currentId) return false;
        return normalizeCompanyName(c?.name?.en || '') === nextKey;
      });
      if (conflictLocal) {
        setError(t('errors.colorNameExists'));
        return;
      }

      const url = isEdit ? `/api/variant-colors/${initialValue._id}` : '/api/variant-colors';
      ...
      if (!res.ok) {
        if (res.status === 409) throw new Error(json?.message || t('errors.colorNameExists'));
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      <DialogTitle>{isEdit ? t('variantColors.edit') : t('variantColors.new')}</DialogTitle>
