...
export async function fetchConsigneeByPhone(phone) {
  ...
  for (const entry of entries) {
    if (entry && typeof entry === 'object' && !Array.isArray(entry) && !entry.fields) {
      const entryPhoneDigits = String(entry['phone'] || '').replace(/\D/g, '') || digits;
      suggestions.push({
        name: String(entry['name'] || ''),
        phone: entryPhoneDigits,
        addressLine: String(entry['address'] || ''),
        cityId: undefined,
        cityName: entry['city.name'] ? String(entry['city.name']) : undefined,
        areaId: undefined,
        areaName: entry['area.name'] ? String(entry['area.name']) : undefined,
      });
    } else {
      const fields = entry?.fields || [];
      const name = fval(fields, 'name').value || '';
      const addressLine = fval(fields, 'address').value || '';
      const phoneField = fval(fields, 'phone').value || '';
      const entryPhoneDigits = String(phoneField || '').replace(/\D/g, '') || digits;
      const city = fval(fields, 'city.name');
      const area = fval(fields, 'area.name');
      suggestions.push({
        name,
        phone: entryPhoneDigits,
        addressLine,
        cityId: city.resource_id != null ? Number(city.resource_id) : undefined,
        cityName: city.value || undefined,
        areaId: area.resource_id != null ? Number(area.resource_id) : undefined,
