...
export async function ensureReceiptEditable(receiptId) {
  await connectToDB();
  const r = await Receipt.findById(receiptId).select({ status: 1, type: 1 }).lean().exec();
  if (!r) {
...
const PURCHASE_ALLOWED_NEXT = {
  ordered: new Set(['on_delivery', 'completed']),
  on_delivery: new Set(['completed']),
  completed: new Set([]),
};

export function assertStatusTransition(current, next, opts = {}) {
  if (!next) {
  ...
  const rules = opts?.type === 'purchase' ? PURCHASE_ALLOWED_NEXT : ALLOWED_NEXT;
  const ok = rules[current]?.has(next);
  if (!ok) {
